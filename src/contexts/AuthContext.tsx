...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    if (sessionId) {
      ...
      if (found) {
        setUser({
          ...
          trialEndsAt:
            found.trialEndsAt || new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString(),
  ...
  const addAudioMinutes = useCallback(
    (minutes: number) => {
      if (!user) return;
      const users = getUsers();
      const idx = users.findIndex((u) => u.id === user.id);
  ...
  const completeOnboarding = useCallback(
    (data: { faculteId: string; prenom: string; age: string; ville: string; avatarId: string }) => {
      setUser((prev) => {
        if (!prev) return prev;
        const updated = {
          ...prev,
