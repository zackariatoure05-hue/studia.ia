...
export function DashboardMain({ children }: { children: React.ReactNode }) {
  ...
  if (isLoggedIn && user) {
    if (!user.hasCompletedOnboarding || (user.status === "trial" && new Date(user.trialEndsAt).getTime() < Date.now())) {
      return null;
    }
  }
  ...
  if (isError) {
    return (
      <div style={{ flex: 1, padding: "2rem 2.5rem", overflowY: "auto", display: "flex", flexDirection: "column", gap: "2rem" }}>
        {/* Un bouton pour reset l'erreur pour la démo */}
        <div>
          <button
            onClick={() => triggerError(false)}
            style={{
              padding: "0.5rem 1rem", background: "var(--border)", border: "none",
