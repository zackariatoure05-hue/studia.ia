    ...
    <div className="w-full aspect-square flex items-center justify-center perspective-[1200px] bg-slate-50 relative overflow-hidden h-full min-h-[400px]">
      ...
      >
        ...
        <div className="absolute inset-0 backface-hidden bg-white border border-slate-200 rounded-3xl p-8 flex flex-col justify-center items-center text-center">
          ...
          <h3 className="font-extrabold text-slate-800 text-2xl mb-4">
            Qu&apos;est-ce que la mitose ?
          </h3>
          <p className="text-sm font-semibold text-slate-400 mt-auto bg-slate-100 px-4 py-2 rounded-full">
            Clique pour retourner
          </p>
        </div>
        {/* Back */}
        <div
          className="absolute inset-0 backface-hidden bg-gradient-to-br from-purple-600 to-pink-600 rounded-3xl p-8 flex flex-col justify-center items-center text-center text-white"
          style={{ transform: "rotateY(180deg)" }}
        >
          <CheckCircle2 className="w-12 h-12 text-white/90 mb-6" />
          <p className="font-bold text-lg text-white/95 leading-relaxed">
            Division cellulaire produisant deux cellules filles génétiquement identiques.
          </p>
        </div>
      </div>
