...
export default function CoursDetailPage() {
  ...
  const toggleCard = (fcId: string) => {
    setFlippedCards((prev) => {
      ...
      if (next.has(fcId)) next.delete(fcId);
      else next.add(fcId);
      return next;
    });
  };
  ...
  if (!c || !matiere || !faculte) {
    return (
      <>
        <Breadcrumb
          items={[{ label: "Cours", href: "/tableau-de-bord" }, { label: "Introuvable" }]}
        />
          ...
          <div className="notebook-paper" style={{ animation: "fadeIn 0.25s" }}>
            {resume ? (
                ...
                >
                  <ReactMarkdown
                    components={{
                      h1: ({ node, ...props }) => (
                        <h1
                          className="text-2xl font-black text-primary mb-4 mt-6 border-b pb-2"
                          {...props}
                        />
                      ),
                      h2: ({ node, ...props }) => (
                        <h2 className="text-xl font-bold text-foreground mb-3 mt-5" {...props} />
                      ),
                      h3: ({ node, ...props }) => (
                        <h3
              ...
              >
                ...
                >
                  Aucun résumé rédigé sur ce cahier.
                </p>
              </div>
            )}
          </div>
