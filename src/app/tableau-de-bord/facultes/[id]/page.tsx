...
export default function FaculteDetailPage() {
  ...
  const { facultes, matieres, cours, addMatiere, renameMatiere, deleteMatiere, isLoading } = useData();
  ...
  if (!faculte) {
    return (
      <>
        <Breadcrumb items={[{ label: "Facultés", href: "/tableau-de-bord/facultes" }, { label: "Introuvable" }]} />
        <div style={{ padding: "3rem 2rem", textAlign: "center" }}>
          <p style={{ fontSize: "2rem", margin: "0 0 0.5rem" }}>😕</p>
          <p style={{ fontWeight: 700, fontSize: "1rem", color: "var(--foreground)", margin: "0 0 0.5rem" }}>Faculté introuvable</p>
  ...
  return (
    <>
      ...
      {/* Formulaire d'ajout */}
      {showAdd && (
        <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: "calc(var(--radius) + 2px)", padding: "1rem 1.25rem", marginBottom: "1rem", display: "flex", alignItems: "center", gap: "0.75rem" }}>
          <input
            autoFocus
            ...
            style={{ flex: 1, padding: "0.5rem 0.75rem", border: "1px solid var(--border)", borderRadius: "var(--radius)", fontSize: "0.9rem", outline: "none", boxSizing: "border-box" }}
          />
            ...
            return (
              <div key={mat.id} style={{
                ...
                <div style={{ display: "flex", gap: "0.4rem", flexShrink: 0 }}>
                  {!isEditing && (
                    <button onClick={() => startEdit(mat.id, mat.nom)} title="Renommer" style={{ background: "none", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "0.35rem 0.5rem", cursor: "pointer", color: "var(--muted-foreground)", display: "flex", alignItems: "center" }}>
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Z"/>
                      </svg>
                    </button>
                  )}
                  <button onClick={() => handleDelete(mat.id, mat.nom)} title="Supprimer" style={{ background: "none", border: "1px solid oklch(0.9 0.1 27)", borderRadius: "var(--radius)", padding: "0.35rem 0.5rem", cursor: "pointer", color: "oklch(0.577 0.245 27.325)", display: "flex", alignItems: "center" }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"/>
                    </svg>
                  </button>
                  <button onClick={() => router.push(`/tableau-de-bord/matiere/${mat.id}`)} title="Voir" style={{ background: "none", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "0.35rem 0.5rem", cursor: "pointer", color: "var(--muted-foreground)", display: "flex", alignItems: "center" }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
